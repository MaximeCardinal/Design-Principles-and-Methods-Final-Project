 ...
 * @version 1.4
 * @editHistory Maxime Cardinal - Creation and implementation of travelToBridge method (03/11/2018) (1.0)
 * 				Maxime Cardinal - Implementation of the travelThroughBridge and travelToRingSet methods (06/11/2018) (1.1)
 * 				Maxime Cardinal - Modification of the travelThroughBridge -> 1x1 bridge case (12/11/2018) (1.2)
 * 				Maxime Cardinal - Implementation of the odometerCorrection method and modification of the travelling 
 * 											by adding some position correction (13/11/2018) (1.3)
 * 				Irmak Pakis - Modification of the traveling algorithm and th correction algorithm (22/11/2018) (1.4)
 *
 */
public class TravelToTree {
	...
	private static final double sensorOffset = -5.3;
	private static final double correctionOffset = -5.0;
	private static final double lineDetection = DesignProjectMain.lineDetection;
	...
	public static void travelThroughBridge() {
		...
		//if the bridge is vertically oriented, travel in y
			...
			if(StartingCorner == 0) {
				...
				travelTo((BR_UR_x), (BR_UR_y-bridgeSize));
				odometerCorrection((BR_UR_x), (BR_UR_y-bridgeSize));
			}
			else if(StartingCorner == 3) {
				travelTo((BR_UR_x-HALF_TILE), (BR_UR_y+HALF_TILE-bridgeSize));
				travelTo((BR_UR_x), (BR_UR_y-bridgeSize));
	...
	public static void travelToRingSet() {
		...
		if(T_x < currentCoordinates[0]/TILE_SIZE && T_y < currentCoordinates[1]/TILE_SIZE || 
					T_x < currentCoordinates[0]/TILE_SIZE && T_y > currentCoordinates[1]/TILE_SIZE) {
			
			travelTo(T_x+1,T_y);
			odometerCorrection(T_x+1,T_y);
	...
	private static void angleCorrection() {
		...
		while (true) {
			...
			if (rightS < lineDetection && !rightDetected) {
				rightMotor.stop(!leftDetected);
				rightDetected = true;
			}
			if (leftS < lineDetection && !leftDetected) {
	...
	/**
	 * This method sleeps the thread
	 *  
	 * @param a: time of sleeping 
	 */
